...
public class RunningServiceManager extends SelfletComponent implements IRunningServiceManager {
	...
	private static final ThreadPoolExecutor MAIN_SERVICE_THREAD_POOL = (ThreadPoolExecutor) Executors.newFixedThreadPool(SERVICE_THREAD_POOL_SIZE,
			new PriorityThreadFactory(SERVICE_THREAD_POOL_PRIORITY, "Main Service Thread"));

	private static final ThreadPoolExecutor CHILD_SERVICE_THREAD_POOL = (ThreadPoolExecutor) Executors.newFixedThreadPool(Integer.MAX_VALUE,
			new PriorityThreadFactory(SERVICE_THREAD_POOL_PRIORITY, "Child Service Thread"));
	...
	private void initEventHandlers() {
		...
		ImmutableSet<EventTypeEnum> serviceEvents = ImmutableSet.of(LOCAL_REQUEST_LOCAL_EXECUTION_EXECUTE, REMOTE_REQUEST_LOCAL_EXECUTION_EXECUTE,
				LOCAL_REQUEST_REMOTE_EXECUTION_EXECUTE, LOCAL_REQUEST_LOCAL_EXECUTION_COMPLETED, REMOTE_REQUEST_LOCAL_EXECUTION_COMPLETED,
	...
	public ImmutableSet<EventTypeEnum> getReceivedEvents() {
		return ImmutableSet.of(LOCAL_REQUEST_LOCAL_EXECUTION_EXECUTE, REMOTE_REQUEST_LOCAL_EXECUTION_EXECUTE, LOCAL_REQUEST_REMOTE_EXECUTION_EXECUTE,
				LOCAL_REQUEST_LOCAL_EXECUTION_COMPLETED, REMOTE_REQUEST_LOCAL_EXECUTION_COMPLETED, LOCAL_REQUEST_REMOTE_EXECUTION_COMPLETED);
	}
	...
	public ServiceExecutionStats getServiceExecutionStats() {
		...
		LOG.debug("Active count: MAIN_SERVICE_THREAD_POOL: " + MAIN_SERVICE_THREAD_POOL.getActiveCount() + " CHILD_SERVICE_THREAD_POOL: "
				+ CHILD_SERVICE_THREAD_POOL.getActiveCount());
		serviceExecutionStats.setActiveCount(activeCount);

		long completed = MAIN_SERVICE_THREAD_POOL.getCompletedTaskCount() + CHILD_SERVICE_THREAD_POOL.getCompletedTaskCount();
		serviceExecutionStats.setCompletedTaskCount(completed);
		serviceExecutionStats.setQueueLength(MAIN_SERVICE_THREAD_POOL.getQueue().size() + MAIN_SERVICE_THREAD_POOL.getQueue().size());
